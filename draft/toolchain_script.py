...
def run_cli_candidate(args_parse):
    ...
    if candidate == 'generic_tests':
        ...
        if set(list_of_tools) == {'ctverif'}:
            target_wrapper = f'{target_basename}_wrapper'
            target_src_file = target_required_src_files[0]
            ctverif_target_src_file = generic.ctverif_create_target_wrapper(target_src_file, target_basename,
                                                                            target_required_include_files)
            generic.run_ctverif(target_wrapper, ctverif_target_src_file)
        if set(list_of_tools) == {'flowtracker'}:
            target_src_file = target_required_src_files[0]
            include_dir = target_include_directories
            xml_file = test_harness
            generic.flowtracker_test_harness_template(target_basename, target_src_file, target_secret_inputs, xml_file)
            generic.flowtracker_compile_target_src_file(target_src_file, include_dir, xml_file)
    else:
