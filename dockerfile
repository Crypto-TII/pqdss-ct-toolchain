...
# Valgrind
RUN apt-get update && \
    apt-get install -y bzip2 libc6-dbg gcc wget git make build-essential libssl-dev libffi-dev python3-pip
...
COPY ctgrind.h /usr/share/ctgrind/
COPY ctgrind.c /usr/share/ctgrind/
COPY ctgrind.h /usr/include/ctgrind.h
